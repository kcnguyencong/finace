  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen pb-24 md:pb-0">
        ...
        <div className="flex items-center gap-sm">
          <div className="relative">
            <button
              ...
              className="w-10 h-10 rounded-full overflow-hidden border-2 border-secondary/20 hover:border-secondary/50 transition-colors cursor-pointer"
            >
              <div className="w-full h-full bg-gradient-to-br from-secondary/30 to-primary-container flex items-center justify-center">
                <span className="text-secondary text-label-lg font-bold uppercase">
            ...
            {/* Logout Dropdown */}
            {showLogout && (
              <div className="absolute top-12 left-0 bg-surface-container-lowest border border-white/[0.06] rounded-xl shadow-2xl shadow-black/40 py-1 min-w-[160px] z-50 animate-in">
                <div className="px-4 py-2 border-b border-white/[0.06]">
                  <p className="text-label-sm text-on-surface-variant truncate">{user?.email}</p>
                </div>
                <button
                  onClick={handleLogout}
                  className="w-full px-4 py-2.5 text-left text-label-sm font-label-sm text-error hover:bg-error-container/10 flex items-center gap-2 transition-colors"
                >
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-headline-md font-headline-md text-on-surface">{displayName}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-container-padding space-y-lg py-sm">
        ...
        <section className="bg-gradient-to-br from-[#0f172a] to-[#1e293b] rounded-[24px] p-lg text-white shadow-xl relative overflow-hidden border border-white/[0.04]">
          ...
          <div className="grid grid-cols-2 gap-sm mt-lg pt-lg border-t border-white/10">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-label-lg font-label-lg text-on-error-container">-{formatCurrency(totalExpense)}</span>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-md pb-8">
          ...
          <div className="space-y-sm">
            {loading ? (
              ...
              <Link to={`/transaction?id=${tx.id}`} key={tx.id} className="bg-surface-container-lowest rounded-xl p-md flex items-center justify-between shadow-[0px_2px_8px_rgba(0,0,0,0.15)] border border-white/[0.04] group active:scale-[0.98] transition-transform cursor-pointer hover:bg-surface-container-low">
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-surface/80 backdrop-blur-xl border-t border-outline-variant/30 flex justify-around items-center px-4 py-2 pb-safe z-50">
        <Link to="/" className="flex flex-col items-center justify-center text-secondary font-bold transition-transform active:scale-90">
          <span className="material-symbols-outlined" style={{fontVariationSettings: "'FILL' 1"}}>home</span>
          <span className="text-label-sm font-label-sm">Trang chủ</span>
        </Link>
        <Link to="/transactions" className="flex flex-col items-center justify-center text-on-surface-variant hover:text-secondary transition-colors transition-transform active:scale-90">
        ...
        <button onClick={handleLogout} className="flex flex-col items-center justify-center text-on-surface-variant hover:text-error transition-colors transition-transform active:scale-90">
